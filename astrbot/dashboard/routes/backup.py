...
class BackupRoute(Route):
        ...
        while True:
            try:
                ...
                for upload_id in expired_sessions:
                    ...
                    logger.info(
                        t(
                            "dashboard-routes-backup-cleaning_expired_upload_session",
                            upload_id=upload_id,
                        )
                    )
                ...
                logger.error(
                    t("dashboard-routes-backup-cleanup_expired_session_failed", e=e)
                )
        ...
        if upload_id in self.upload_sessions:
            ...
            if chunk_dir and os.path.exists(chunk_dir):
                try:
                    ...
                    logger.warning(
                        t("dashboard-routes-backup-cleanup_chunk_directory_failed", e=e)
                    )
    ...
    def _get_backup_manifest(self, zip_path: str) -> dict | None:
        ...
        try:
            ...
            logger.debug(t("dashboard-routes-backup-read_backup_manifest_failed", e=e))
        ...
        try:
            ...
            for filename in os.listdir(self.backup_dir):
                ...
                if manifest is None:
                    logger.debug(
                        t(
                            "dashboard-routes-backup-skipping_invalid_backup_file",
                            filename=filename,
                        )
                    )
                    continue
                ...
                backup_files.append(
                    {
                        ...
                        "astrbot_version": manifest.get(
                            "astrbot_version",
                            t("dashboard-routes-backup-astrbot_version_field"),
                        ),
            ...
            logger.error(t("dashboard-routes-backup-get_backup_list_failed", e=e))
            logger.error(traceback.format_exc())
            return (
                Response()
                .error(t("dashboard-routes-backup-error_get_backup_list_failed", e=e))
                .__dict__
            )
    ...
    async def export_backup(self):
        ...
        try:
            ...
            logger.error(t("dashboard-routes-backup-create_backup_failed", e=e))
            logger.error(traceback.format_exc())
            return (
                Response()
                .error(t("dashboard-routes-backup-create_backup_failed", e=e))
                .__dict__
            )
    ...
    async def _background_export_task(self, task_id: str) -> None:
        ...
        try:
            self._update_progress(
                task_id,
                status="processing",
                message=t("dashboard-routes-backup-update_progress_initializing"),
            )
            ...
            logger.error(
                t(
                    "dashboard-routes-backup-background_export_task_failed",
                    task_id=task_id,
                    e=e,
                )
            )
    ...
    async def upload_backup(self):
        ...
        try:
            ...
            if "file" not in files:
                return (
                    Response()
                    .error(t("dashboard-routes-backup-missing_backup_file"))
                    .__dict__
                )
            ...
            if not file.filename or not file.filename.endswith(".zip"):
                return (
                    Response()
                    .error(t("dashboard-routes-backup-please_upload_zip_format"))
                    .__dict__
                )
            ...
            logger.info(
                t(
                    "dashboard-routes-backup-uploaded_file_saved",
            ...
            logger.error(t("dashboard-routes-backup-upload_backup_file_failed", e=e))
            logger.error(traceback.format_exc())
            return (
                Response()
                .error(t("dashboard-routes-backup-upload_backup_failed", e=e))
                .__dict__
            )
        ...
        try:
            ...
            if not filename:
                return (
                    Response()
                    .error(
                        t("dashboard-routes-backup-hardcoded-error_missing_filename")
                    )
                    .__dict__
                )
            ...
            if not filename.endswith(".zip"):
                return (
                    Response()
                    .error(
                        t("dashboard-routes-backup-hardcoded-error_zip_format_required")
                    )
                    .__dict__
                )
            ...
            if total_size <= 0:
                return (
                    Response()
                    .error(
                        t("dashboard-routes-backup-hardcoded-error_invalid_file_size")
                    )
                    .__dict__
                )
            ...
            logger.info(
                t(
                    "dashboard-routes-backup-hardcoded-log_init_multipart_upload",
                    upload_id=upload_id,
                )
            ...
            logger.error(
                t("dashboard-routes-backup-hardcoded-log_init_multipart_failed", e=e)
            )
            logger.error(traceback.format_exc())
            return (
                Response()
                .error(t("dashboard-routes-backup-init_chunk_upload_failed", e=e))
                .__dict__
            )
        ...
        try:
            ...
            if not upload_id or chunk_index_str is None:
                return (
                    ...
                    .error(
                        t(
                            "dashboard-routes-backup-hardcoded-error_missing_required_params"
                        )
                    )
                    .__dict__
                )
            ...
            try:
                ...
                return (
                    Response()
                    .error(
                        t("dashboard-routes-backup-hardcoded-error_invalid_chunk_index")
                    )
                    .__dict__
                )
            ...
            if "chunk" not in files:
                return (
                    Response()
                    .error(
                        t("dashboard-routes-backup-hardcoded-error_missing_chunk_data")
                    )
                    .__dict__
                )
            ...
            if upload_id not in self.upload_sessions:
                return (
                    ...
                    .error(
                        t(
                            "dashboard-routes-backup-hardcoded-error_upload_session_invalid"
                        )
                    )
                    .__dict__
                )
            ...
            if chunk_index < 0 or chunk_index >= session["total_chunks"]:
                return (
                    ...
                    .error(
                        t(
                            "dashboard-routes-backup-hardcoded-error_chunk_index_out_of_range"
                        )
                    )
                    .__dict__
                )
            ...
            logger.debug(
                t(
                    "dashboard-routes-backup-hardcoded-log_receiving_chunk",
                    upload_id=upload_id,
                )
            ...
            logger.error(
                t("dashboard-routes-backup-hardcoded-log_upload_chunk_failed", e=e)
            )
            logger.error(traceback.format_exc())
            return (
                Response()
                .error(t("dashboard-routes-backup-chunk_upload_failed", e=e))
                .__dict__
            )
    ...
    def _mark_backup_as_uploaded(self, zip_path: str) -> None:
        ...
        try:
            ...
            logger.debug(
                t(
                    "dashboard-routes-backup-hardcoded-log_mark_backup_source_success",
                    zip_path=zip_path,
                )
            )
        except Exception as e:
            logger.warning(
                t(
                    "dashboard-routes-backup-hardcoded-log_mark_backup_source_failed",
                    e=e,
                )
            )
        ...
        try:
            ...
            if not upload_id:
                return (
                    Response()
                    .error(
                        t("dashboard-routes-backup-hardcoded-error_missing_upload_id")
                    )
                    .__dict__
                )
            ...
            if upload_id not in self.upload_sessions:
                return (
                    ...
                    .error(
                        t(
                            "dashboard-routes-backup-hardcoded-error_session_not_found_or_expired"
                        )
                    )
                    .__dict__
                )
            ...
            if len(received) != total:
                ...
                return (
                    ...
                    .error(
                        t(
                            "dashboard-routes-backup-incomplete_shard_missing_keys",
            ...
            try:
                ...
                logger.info(
                    t(
                        "dashboard-routes-backup-chunked_upload_completed",
                        filename=filename,
                        file_size=file_size,
                        total=total,
                    )
            ...
            logger.error(
                t("dashboard-routes-backup-complete_chunked_upload_failed", e=e)
            )
            logger.error(traceback.format_exc())
            return (
                Response()
                .error(t("dashboard-routes-backup-complete_chunk_upload_failed", e=e))
                .__dict__
            )
    ...
    async def upload_abort(self):
        ...
        try:
            ...
            if not upload_id:
                return (
                    Response()
                    .error(t("dashboard-routes-backup-missing_upload_id"))
                    .__dict__
                )
            ...
            if upload_id not in self.upload_sessions:
                ...
                return (
                    Response()
                    .ok(message=t("dashboard-routes-backup-upload_canceled_success"))
                    .__dict__
                )
            ...
            logger.info(
                t("dashboard-routes-backup-cancel_chunked_upload", upload_id=upload_id)
            )
            ...
            return (
                ...
                .ok(
                    message=t(
                        "dashboard-routes-backup-upload_canceled_success_response"
                    )
                )
                .__dict__
            )
        except Exception as e:
            logger.error(t("dashboard-routes-backup-cancel_upload_failed", e=e))
            logger.error(traceback.format_exc())
            return (
                Response()
                .error(t("dashboard-routes-backup-cancel_upload_failed", e=e))
                .__dict__
            )
    ...
    async def check_backup(self):
        ...
        try:
            ...
            if not filename:
                return (
                    Response()
                    .error(t("dashboard-routes-backup-missing_filename"))
                    .__dict__
                )
            ...
            if ".." in filename or "/" in filename or "\\" in filename:
                return (
                    Response()
                    .error(t("dashboard-routes-backup-invalid_filename"))
                    .__dict__
                )
            ...
            if not os.path.exists(zip_path):
                return (
                    ...
                    .error(
                        t(
                            "dashboard-routes-backup-backup_file_not_found",
                            filename=filename,
                        )
                    )
                    .__dict__
            ...
            logger.error(t("dashboard-routes-backup-precheck_backup_failed", e=e))
            logger.error(traceback.format_exc())
            return (
                Response()
                .error(t("dashboard-routes-backup-precheck_backup_failed", e=e))
                .__dict__
            )
    ...
    async def import_backup(self):
        ...
        try:
            ...
            if not filename:
                return (
                    Response()
                    .error(t("dashboard-routes-backup-missing_filename_import"))
                    .__dict__
                )
            ...
            if not confirmed:
                return (
                    Response()
                    .error(t("dashboard-routes-backup-import_confirmation_required"))
                    .__dict__
                )
            ...
            if ".." in filename or "/" in filename or "\\" in filename:
                return (
                    Response()
                    .error(t("dashboard-routes-backup-invalid_filename_import"))
                    .__dict__
                )
            ...
            if not os.path.exists(zip_path):
                return (
                    ...
                    .error(
                        t(
                            "dashboard-routes-backup-backup_file_not_found_import",
                            filename=filename,
                        )
                    )
                    .__dict__
            ...
            logger.error(t("dashboard-routes-backup-import_backup_failed", e=e))
            logger.error(traceback.format_exc())
            return (
                Response()
                .error(t("dashboard-routes-backup-import_backup_failed", e=e))
                .__dict__
            )
    ...
    async def _background_import_task(self, task_id: str, zip_path: str) -> None:
        ...
        try:
            ...
            logger.error(
                t(
                    "dashboard-routes-backup-logger_import_task_failed",
                    task_id=task_id,
                    e=e,
                )
            )
    ...
    async def get_progress(self):
        ...
        try:
            ...
            if not task_id:
                return (
                    Response()
                    .error(t("dashboard-routes-backup-error_missing_task_id"))
                    .__dict__
                )
            ...
            if task_id not in self.backup_tasks:
                return (
                    Response()
                    .error(t("dashboard-routes-backup-error_task_not_found"))
                    .__dict__
                )
            ...
            logger.error(t("dashboard-routes-backup-logger_get_progress_failed", e=e))
            logger.error(traceback.format_exc())
            return (
                Response()
                .error(t("dashboard-routes-backup-get_task_progress_failed", e=e))
                .__dict__
            )
    ...
    async def download_backup(self):
        ...
        try:
            ...
            if not filename:
                return (
                    Response()
                    .error(t("dashboard-routes-backup-error_missing_filename"))
                    .__dict__
                )
            ...
            if not token:
                return (
                    Response()
                    .error(t("dashboard-routes-backup-error_missing_token"))
                    .__dict__
                )
            ...
            try:
                ...
                if not jwt_secret:
                    return (
                        Response()
                        .error(t("dashboard-routes-backup-error_server_config"))
                        .__dict__
                    )
                ...
                return (
                    Response()
                    .error(t("dashboard-routes-backup-error_token_expired"))
                    .__dict__
                )
            except jwt.InvalidTokenError:
                return (
                    Response()
                    .error(t("dashboard-routes-backup-error_invalid_token"))
                    .__dict__
                )
            ...
            if ".." in filename or "/" in filename or "\\" in filename:
                return (
                    Response()
                    .error(t("dashboard-routes-backup-error_invalid_filename"))
                    .__dict__
                )
            ...
            if not os.path.exists(file_path):
                return (
                    Response()
                    .error(t("dashboard-routes-backup-error_backup_not_exist"))
                    .__dict__
                )
            ...
            logger.error(
                t("dashboard-routes-backup-logger_download_backup_failed", e=e)
            )
            logger.error(traceback.format_exc())
            return (
                Response()
                .error(t("dashboard-routes-backup-download_backup_failed", e=e))
                .__dict__
            )
    ...
    async def delete_backup(self):
        ...
        try:
            ...
            if not filename:
                return (
                    Response()
                    .error(t("dashboard-routes-backup-error_missing_filename_download"))
                    .__dict__
                )
            ...
            if ".." in filename or "/" in filename or "\\" in filename:
                return (
                    Response()
                    .error(t("dashboard-routes-backup-error_invalid_filename_download"))
                    .__dict__
                )
            ...
            if not os.path.exists(file_path):
                return (
                    Response()
                    .error(t("dashboard-routes-backup-error_backup_not_exist_download"))
                    .__dict__
                )
            ...
            return (
                Response()
                .ok(message=t("dashboard-routes-backup-success_backup_deleted"))
                .__dict__
            )
        except Exception as e:
            logger.error(t("dashboard-routes-backup-logger_delete_backup_failed", e=e))
            logger.error(traceback.format_exc())
            return (
                Response()
                .error(t("dashboard-routes-backup-delete_backup_failed", e=e))
                .__dict__
            )
    ...
    async def rename_backup(self):
        ...
        try:
            ...
            if not filename:
                return (
                    Response()
                    .error(t("dashboard-routes-backup-missing_filename_dup2"))
                    .__dict__
                )
            ...
            if not new_name:
                return (
                    Response()
                    .error(t("dashboard-routes-backup-missing_new_name"))
                    .__dict__
                )
            ...
            if not new_name or new_name.replace("_", "") == "":
                return (
                    Response()
                    .error(t("dashboard-routes-backup-invalid_new_filename"))
                    .__dict__
                )
            ...
            if not os.path.exists(old_path):
                return (
                    Response()
                    .error(t("dashboard-routes-backup-backup_not_found"))
                    .__dict__
                )
            ...
            if os.path.exists(new_path):
                return (
                    ...
                    .error(
                        t(
                            "dashboard-routes-backup-filename_exists",
            ...
            logger.info(
                t(
                    "dashboard-routes-backup-backup_renamed",
                    filename=filename,
                    new_filename=new_filename,
                )
            )
            ...
            logger.error(t("dashboard-routes-backup-rename_backup_failed_log", e=e))
            logger.error(traceback.format_exc())
            return (
                Response()
                .error(t("dashboard-routes-backup-rename_backup_failed", e=e))
                .__dict__
            )
