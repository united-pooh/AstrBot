...
class AstrBotExporter:
    ...
    async def export_all(
        ...
        logger.info(t('backup-exporter-info_start_export_backup', zip_path=zip_path))

        try:
            with zipfile.ZipFile(zip_path, "w", zipfile.ZIP_DEFLATED) as zf:
                # 1. 导出主数据库
                if progress_callback:
                    await progress_callback("main_db", 0, 100, t('backup-exporter-progress_exporting_main_db'))
                ...
                if progress_callback:
                    await progress_callback("main_db", 100, 100, t('backup-exporter-progress_main_db_export_complete'))
                ...
                if self.kb_manager:
                    if progress_callback:
                        await progress_callback(
                            "kb_metadata", 0, 100, t('backup-exporter-exporting_kb_metadata')
                        )
                    ...
                    if progress_callback:
                        await progress_callback(
                            "kb_metadata", 100, 100, t('backup-exporter-kb_metadata_export_completed')
                        )
                    ...
                    if progress_callback:
                        await progress_callback(
                            "kb_documents", total_kbs, total_kbs, t('backup-exporter-kb_documents_export_completed')
                        )

                # 3. 导出配置文件
                if progress_callback:
                    await progress_callback("config", 0, 100, t('backup-exporter-exporting_config'))
                ...
                if progress_callback:
                    await progress_callback("config", 100, 100, t('backup-exporter-config_export_completed'))

                # 4. 导出附件文件
                if progress_callback:
                    await progress_callback("attachments", 0, 100, t('backup-exporter-exporting_attachments'))
                await self._export_attachments(zf, main_data.get("attachments", []))
                if progress_callback:
                    await progress_callback("attachments", 100, 100, t('backup-exporter-attachments_export_completed'))

                # 5. 导出插件和其他目录
                if progress_callback:
                    await progress_callback(
                        "directories", 0, 100, t('backup-exporter-exporting_directories')
                    )
                dir_stats = await self._export_directories(zf)
                if progress_callback:
                    await progress_callback("directories", 100, 100, t('backup-exporter-directories_export_completed'))

                # 6. 生成 manifest
                if progress_callback:
                    await progress_callback("manifest", 0, 100, t('backup-exporter-generating_manifest'))
                manifest = self._generate_manifest(main_data, kb_meta_data, dir_stats)
                manifest_json = json.dumps(manifest, ensure_ascii=False, indent=2)
                zf.writestr("manifest.json", manifest_json)
                if progress_callback:
                    await progress_callback("manifest", 100, 100, t('backup-exporter-manifest_generation_completed'))

            logger.info(t('backup-exporter-backup_export_completed', zip_path=zip_path))
            return zip_path

        except Exception as e:
            logger.error(t('backup-exporter-backup_export_failed', e=e))
            # 清理失败的文件
        ...
        async with self.main_db.get_db() as session:
            for table_name, model_class in MAIN_DB_MODELS.items():
                try:
                    ...
                    logger.warning(t('backup-exporter-export_table_failed', table_name=table_name, e=e))
                    export_data[table_name] = []
        ...
        async with self.kb_manager.kb_db.get_db() as session:
            for table_name, model_class in KB_METADATA_MODELS.items():
                try:
                    ...
                    logger.warning(t('backup-exporter-export_kb_table_failed', table_name=table_name, e=e))
                    export_data[table_name] = []
    ...
    async def _export_kb_documents(self, kb_helper: Any) -> dict[str, Any]:
        ...
        try:
            ...
            logger.warning(t('backup-exporter-export_kb_documents_failed', e=e))
            return {"documents": []}
    ...
    async def _export_faiss_index(
        ...
        try:
            ...
            if index_path.exists():
                ...
                logger.debug(t('backup-exporter-exported_faiss_index', archive_path=archive_path))
        except Exception as e:
            logger.warning(t('backup-exporter-failed_export_faiss_index', e=e))
    ...
    async def _export_kb_media_files(
        ...
        try:
            ...
            logger.warning(t('backup-exporter-failed_export_media_files', e=e))
    ...
    async def _export_directories(
        ...
        for dir_name, dir_path in backup_directories.items():
            ...
            if not full_path.exists():
                logger.debug(t('backup-exporter-directory_not_exist_skip', full_path=full_path))
                continue
                ...
                for root, dirs, files in os.walk(full_path):
                    ...
                    for file in files:
                        ...
                        try:
                            ...
                            logger.warning(t('backup-exporter-failed_export_file', file_path=file_path, e=e))

                stats[dir_name] = {"files": file_count, "size": total_size}
                logger.debug(
                    t('backup-exporter-exporting_directory_summary', dir_name=dir_name, file_count=file_count, total_size=total_size)
                )
            except Exception as e:
                logger.warning(t('backup-exporter-failed_export_directory', dir_path=dir_path, e=e))
    ...
    async def _export_attachments(
        ...
        for attachment in attachments:
            try:
                ...
                logger.warning(t('backup-exporter-failed_export_attachments', e=e))
