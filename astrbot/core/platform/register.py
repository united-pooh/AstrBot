...
def register_platform_adapter(
    ...
    def decorator(cls):
        if adapter_name in platform_cls_map:
            raise ValueError(
                t(
                    "core-platform-register-adapter_already_registered",
        ...
        logger.debug(
            t(
                "core-platform-register-adapter_registered_debug",
                adapter_name=adapter_name,
            )
        )
        return cls
...
def unregister_platform_adapters_by_module(module_path_prefix: str) -> list[str]:
    ...
    for pm in to_remove:
        ...
        logger.debug(t("core-platform-register-adapter_unregistered", pm=pm))
