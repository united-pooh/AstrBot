...
async def toggle_command(handler_full_name: str, enabled: bool) -> CommandDescriptor:
    ...
    if not descriptor:
        raise ValueError(t("core-star-command_management-handler_not_found"))
...
async def rename_command(
    ...
    if not new_fragment:
        raise ValueError(t("core-star-command_management-name_empty"))
    ...
    if _is_command_in_use(handler_full_name, candidate_full):
        raise ValueError(
            t(
                "core-star-command_management-command_name_conflict",
    ...
    if aliases:
        for alias in aliases:
            ...
            if _is_command_in_use(handler_full_name, alias_full):
                raise ValueError(
                    t(
                        "core-star-command_management-alias_conflict",
...
async def update_command_permission(
    ...
    if permission_type not in ["admin", "member"]:
        raise ValueError(t("core-star-command_management-invalid_permission_type"))

    handler = descriptor.handler
    found_plugin = star_map.get(handler.handler_module_path)
    if not found_plugin:
        raise ValueError(t("core-star-command_management-plugin_not_found"))
...
def _collect_descriptors(include_sub_commands: bool) -> list[CommandDescriptor]:
    ...
    for handler in star_handlers_registry:
        try:
            ...
            logger.warning(
                t(
                    "core-star-command_management-handler_parse_failed",
                    handler=handler,
                    e=e,
                )
            )
