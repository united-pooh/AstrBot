...
class AstrBotUpdator(RepoZipUpdator):
    ...
    def terminate_child_processes(self) -> None:
        ...
        try:
            ...
            logger.info(t("core-updator-log_terminating_children", children=children))
            for child in children:
                logger.info(t("core-updator-log_terminating_child_pid", child=child))
                child.terminate()
                try:
                    ...
                    logger.info(
                        t(
                            "core-updator-log_child_not_terminated_force_kill",
                            child=child,
                        )
                    )
                    child.kill()
    ...
    def _reboot(self, delay: int = 3) -> None:
        ...
        try:
            ...
            logger.error(
                t("core-updator-log_restart_failed_manual", executable=executable, e=e)
            )
            raise e
    ...
    async def update(self, reboot=False, latest=True, version=None, proxy="") -> None:
        ...
        if os.environ.get("ASTRBOT_CLI") or os.environ.get("ASTRBOT_LAUNCHER"):
            raise Exception(
                t("core-updator-raise_unsupported_start_method")
            )  # 避免版本管理混乱

        if latest:
            latest_version = update_data[0]["tag_name"]
            if self.compare_version(VERSION, latest_version) >= 0:
                raise Exception(t("core-updator-raise_already_latest_version"))
            ...
            if not file_url:
                raise Exception(
                    t("core-updator-raise_version_file_not_found", version=version)
                )
        else:
            if len(str(version)) != 40:
                raise Exception(t("core-updator-raise_invalid_commit_hash_length"))
            file_url = f"https://github.com/AstrBotDevs/AstrBot/archive/{version}.zip"
        logger.info(t("core-updator-log_preparing_core_update", version=version))
        ...
        try:
            ...
            logger.info(t("core-updator-log_core_download_complete_extracting"))
