        ...
        async with self.lock:
            ...
            if not os.path.exists(local_path):
                raise FileNotFoundError(
                    t('file_token_service-file_not_found', local_path=local_path, file_path=file_path),
                )
    ...
    async def handle_file(self, file_token: str) -> str:
        ...
        async with self.lock:
            ...
            if file_token not in self.staged_files:
                raise KeyError(t('file_token_service-raise_invalid_or_expired_token', file_token=file_token))

            file_path, _ = self.staged_files.pop(file_token)
            if not os.path.exists(file_path):
                raise FileNotFoundError(t('file_token_service-raise_file_not_found', file_path=file_path))
            return file_path
