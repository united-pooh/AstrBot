...
builtin-stars-web-searcher-link-summary-instruction = Pour la question, résumez les points clés à partir des résultats ci-dessus, puis ajoutez des liens de référence à la fin quand ils sont disponibles.
...
builtin-stars-plugin-list-footer = Utilisez /plugin help <plugin_name> pour voir l'aide du plugin et ses commandes enregistrées.Utilisez /plugin on/off <plugin_name> pour activer ou désactiver un plugin.
...
builtin-stars-plugin-help-author-version = ✨ Auteur : {$author}✨ Version : {$version}
builtin-stars-plugin-help-command-list-title = 🔧 Liste des commandes :
...
builtin-stars-plugin-help-command-tip = Astuce : ajoutez le préfixe de réveil pour déclencher les commandes, par défaut /.
...
builtin-stars-provider-list-tts-title = ## Fournisseurs TTS chargés
builtin-stars-provider-list-stt-title = ## Fournisseurs STT chargés
builtin-stars-provider-list-llm-switch-tip = Utilisez /provider <index> pour changer de fournisseur LLM.
builtin-stars-provider-list-tts-switch-tip = Utilisez /provider tts <index> pour changer de fournisseur TTS.
builtin-stars-provider-list-stt-switch-tip = Utilisez /provider stt <index> pour changer de fournisseur STT.
builtin-stars-provider-list-reachability-skipped = La vérification d'accessibilité des fournisseurs a été ignorée. Activez-la dans la configuration si nécessaire.
...
builtin-stars-provider-model-current = Modèle actuel : [{$current_model}]
builtin-stars-provider-model-switch-tip = Astuce : utilisez /model <nom_modele/index> pour changer de modèle en temps réel. Si le modèle cible n'est pas listé, saisissez directement son nom.
...
builtin-stars-provider-key-current = Clé actuelle : {$current_key}
builtin-stars-provider-model-current-inline = Modèle actuel : {$current_model}
builtin-stars-provider-key-switch-tip = Utilisez /key <idx> pour changer de clé.
...
builtin-stars-sid-base-info = UMO : "{$sid}" Cette valeur peut être utilisée pour les paramètres de liste blanche.UID : "{$user_id}" Cette valeur peut être utilisée pour les paramètres d'administrateur.Informations source de session de message : ID bot : "{$umo_platform}" Type de message : "{$umo_msg_type}" ID de session : "{$umo_session_id}"Ces informations source peuvent être utilisées pour la configuration du routage.
builtin-stars-sid-unique-session-group-tip = Le mode session unique est actuellement activé. ID du groupe : "{$group_id}". Vous pouvez aussi ajouter cet ID à la liste blanche pour autoriser tout le groupe.
...
builtin-stars-alter-cmd-usage = Cette commande définit les permissions d'une commande ou d'un groupe de commandes.Format : /alter_cmd <cmd_name> <admin/member>Exemple 1 : /alter_cmd c1 admin définit c1 comme commande admin uniquementExemple 2 : /alter_cmd g1 c1 admin définit la sous-commande c1 du groupe g1 comme admin uniquement/alter_cmd reset config ouvre la configuration des permissions reset
builtin-stars-alter-cmd-reset-config-menu = Configuration fine des permissions pour la commande resetConfiguration actuelle :1. Groupe + session unique ACTIVÉE : {$group_unique_on}2. Groupe + session unique DÉSACTIVÉE : {$group_unique_off}3. Chat privé : {$private}Format de mise à jour :/alter_cmd reset scene <scene_index> <admin/member>Exemple : /alter_cmd reset scene 2 member
...
builtin-stars-persona-overview = [Persona]- Liste des personas : `/persona list`- Définir un persona : `/persona <persona_name>`- Détails d'un persona : `/persona view <persona_name>`- Retirer le persona : `/persona unset`Persona par défaut : {$default_persona_name}Persona de la conversation actuelle {$curr_cid_title} : {$curr_persona_name}Configurez les personas dans WebUI -> page Config
builtin-stars-persona-list-title = 📂 Liste des personas :
builtin-stars-persona-list-total = Total des personas : {$total_count}
builtin-stars-persona-list-set-tip = *Utilisez `/persona <persona_name>` pour définir un persona
...
builtin-stars-conversation-history-result = Historique de la conversation actuelle : {$history}Page {$page} | Total {$total_pages}*Entrez /history 2 pour aller à la page 2
...
builtin-stars-conversation-list-line = {$index}. {$title}({$cid}) Persona : {$persona_id} Mis à jour le : {$updated_at}
builtin-stars-conversation-list-divider = ---
builtin-stars-conversation-current-with-id = Conversation actuelle : {$title}({$cid})
builtin-stars-conversation-current-none = Conversation actuelle : Aucune
builtin-stars-conversation-scope-personal = Portée d'isolation de session : Personnelle
builtin-stars-conversation-scope-group = Portée d'isolation de session : Groupe
builtin-stars-conversation-page-info = Page {$page} | Total {$total_pages}
builtin-stars-conversation-page-jump-tip = *Entrez /ls 2 pour aller à la page 2
...
builtin_stars-astrbot-metadata-desc = desc: Plugin intégré à AstrBot, incluant la mise en œuvre de fonctions telles que l'injection de personnalité, l'injection de contenu de réflexion, la perception du contexte dans les discussions de groupe, etc. Une fois désactivé, ces fonctionnalités ne pourront plus être utilisées.
builtin_stars-builtin_commands-commands-utils-rst_scene-group_unique_on = Séparation par groupe + session activée
builtin_stars-builtin_commands-commands-utils-rst_scene-group_unique_off = Séparation par groupe + session désactivée
builtin_stars-builtin_commands-commands-utils-rst_scene-private = Message privé
builtin_stars-builtin_commands-metadata-desc = desc: Commandes intégrées à AstrBot, offrant des fonctionnalités courantes de gestion de conversation, d'utilisation d'outils, de gestion de plugins, etc.
builtin_stars-session_controller-main-social_media_wakeup_note = Note : vous êtes en train de discuter avec un utilisateur sur les réseaux sociaux. L'utilisateur vous a simplement mentionné avec @ pour vous réveiller, mais n'a pas saisi de contenu dans ce message. Il risque de vous envoyer le contenu qu'il souhaite dans le message suivant.
builtin_stars-session_controller-main-ask_user_intent_friendly = Vous demandez aimablement à l'utilisateur de quoi il veut parler ou de quelle aide il a besoin. La réponse doit être cohérente avec votre personnage et ne pas sembler trop mécanique.
builtin_stars-session_controller-main-output_only_reply_content = Veuillez noter que vous devez uniquement produire le contenu de la réponse à l'utilisateur, rien d'autre.
builtin_stars-session_controller-metadata-desc = desc: Ajoute le support du contrôle de session pour les plugins
builtin_stars-web_searcher-main-migrate_old_tavily_key = Détection d'une ancienne clé websearch_tavily_key (format chaîne), migration automatique vers le format liste et enregistrement.
builtin_stars-web_searcher-main-missing_tavily_key_error = Erreur : la clé API Tavily n'est pas configurée dans AstrBot.
builtin_stars-web_searcher-main-command_deprecated_use_webui = Cette commande est obsolète, veuillez activer ou désactiver la fonction de recherche web dans l'interface WebUI.
builtin_stars-web_searcher-main-summarize_with_bullets_and_links = \n\nPour la question posée, veuillez résumer les résultats ci-dessus sous forme de points, et ajouter à la fin les liens de référence correspondants (s'il y en a).
builtin_stars-web_searcher-main-missing_bocha_key_error = Erreur : la clé API BoCha n'est pas configurée dans AstrBot.
builtin_stars-web_searcher-metadata-desc = desc: Permet au LLM de disposer de capacités de recherche sur le web
builtin_stars-session_controller-main-friendly_inquiry = Vous demandez gentiment à l'utilisateur de quoi il souhaite parler ou de quelle aide il a besoin. Votre réponse doit être cohérente avec votre personnage et ne pas sembler trop mécanique.
builtin_stars-session_controller-main-reply_only_instruction = Veuillez noter que vous devez uniquement produire le contenu de la réponse à l'utilisateur, sans rien ajouter d'autre.
builtin_stars-web_searcher-main-tavily_key_migration = Ancienne clé websearch_tavily_key (format chaîne) détectée, migration automatique vers le format liste et sauvegarde effectuée.
builtin_stars-web_searcher-main-tavily_key_missing = Erreur : la clé API Tavily n'est pas configurée dans AstrBot.
builtin_stars-web_searcher-main-command_deprecated = Cette commande est obsolète, veuillez activer ou désactiver la fonction de recherche web dans l'interface WebUI.
builtin_stars-web_searcher-main-summary_instruction = \n\nPour la question posée, veuillez résumer les résultats ci-dessus sous forme de points, et ajouter à la fin les liens de référence correspondants (s'ils existent).
builtin_stars-web_searcher-main-bocha_key_missing = Erreur : la clé API BoCha n'est pas configurée dans AstrBot.
