...
builtin-stars-web-searcher-link-summary-instruction = {"\u000A"}{"\u000A"}По этому вопросу кратко изложите ключевые моменты на основе результатов выше и, при наличии, добавьте в конце ссылки-источники.
...
builtin-stars-plugin-list-footer = {"\u000A"}Используйте /plugin help <plugin_name>, чтобы посмотреть справку и зарегистрированные команды плагина.{"\u000A"}Используйте /plugin on/off <plugin_name>, чтобы включить или отключить плагин.
...
builtin-stars-plugin-help-command-tip = {"\u000A"}Подсказка: чтобы вызвать команду, добавьте префикс пробуждения, по умолчанию /.
...
builtin-stars-provider-list-tts-title = {"\u000A"}## Загруженные TTS-провайдеры{"\u000A"}
builtin-stars-provider-list-stt-title = {"\u000A"}## Загруженные STT-провайдеры{"\u000A"}
builtin-stars-provider-list-llm-switch-tip = {"\u000A"}Используйте /provider <index> для переключения LLM-провайдера.
builtin-stars-provider-list-tts-switch-tip = {"\u000A"}Используйте /provider tts <index> для переключения TTS-провайдера.
builtin-stars-provider-list-stt-switch-tip = {"\u000A"}Используйте /provider stt <index> для переключения STT-провайдера.
builtin-stars-provider-list-reachability-skipped = {"\u000A"}Проверка доступности провайдеров была пропущена. При необходимости включите ее в конфигурации.
...
builtin-stars-provider-model-switch-tip = {"\u000A"}Подсказка: используйте /model <model_name/index> для переключения модели в реальном времени. Если нужной модели нет в списке, введите ее имя напрямую.
...
builtin-stars-sid-base-info = UMO: "{$sid}" Это значение можно использовать для настроек белого списка.{"\u000A"}UID: "{$user_id}" Это значение можно использовать для настроек администратора.{"\u000A"}Информация об источнике сессии сообщения:{"\u000A"}  ID бота: "{$umo_platform}"{"\u000A"}  Тип сообщения: "{$umo_msg_type}"{"\u000A"}  ID сессии: "{$umo_session_id}"{"\u000A"}Эту информацию об источнике можно использовать для настройки маршрутизации.
builtin-stars-sid-unique-session-group-tip = {"\u000A"}{"\u000A"}Сейчас включен режим уникальной сессии. ID группы: "{$group_id}". Вы также можете добавить этот ID в белый список, чтобы разрешить всю группу.
...
builtin-stars-alter-cmd-usage = Эта команда задает права для команды или группы команд.{"\u000A"}Формат: /alter_cmd <cmd_name> <admin/member>{"\u000A"}Пример 1: /alter_cmd c1 admin делает c1 командой только для админа{"\u000A"}Пример 2: /alter_cmd g1 c1 admin делает подкоманду c1 в группе g1 командой только для админа{"\u000A"}/alter_cmd reset config открывает настройки прав reset
builtin-stars-alter-cmd-reset-config-menu = Тонкая настройка прав для команды reset{"\u000A"}Текущая конфигурация:{"\u000A"}1. Группа + уникальная сессия ВКЛ: {$group_unique_on}{"\u000A"}2. Группа + уникальная сессия ВЫКЛ: {$group_unique_off}{"\u000A"}3. Личный чат: {$private}{"\u000A"}Формат обновления:{"\u000A"}/alter_cmd reset scene <scene_index> <admin/member>{"\u000A"}Пример: /alter_cmd reset scene 2 member
...
builtin-stars-persona-overview = [Persona]{"\u000A"}{"\u000A"}- Список persona: `/persona list`{"\u000A"}- Установить persona: `/persona <persona_name>`{"\u000A"}- Детали persona: `/persona view <persona_name>`{"\u000A"}- Сбросить persona: `/persona unset`{"\u000A"}{"\u000A"}Persona по умолчанию: {$default_persona_name}{"\u000A"}Persona текущего диалога {$curr_cid_title}: {$curr_persona_name}{"\u000A"}{"\u000A"}Настройте persona в WebUI -> страница Config{"\u000A"}
builtin-stars-persona-list-title = 📂 Список persona:{"\u000A"}
builtin-stars-persona-list-total = {"\u000A"}Всего persona: {$total_count}
builtin-stars-persona-list-set-tip = {"\u000A"}*Используйте `/persona <persona_name>` для установки persona
...
builtin-stars-conversation-history-result = История текущего диалога: {$history}{"\u000A"}{"\u000A"}Страница {$page} | Всего {$total_pages}{"\u000A"}*Введите /history 2 для перехода на страницу 2
