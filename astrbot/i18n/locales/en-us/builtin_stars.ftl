...
builtin-stars-web-searcher-link-summary-instruction = For the question, summarize key points based on the results above, and append reference links at the end when available.
...
builtin-stars-plugin-list-footer = Use /plugin help <plugin_name> to view plugin help and registered commands.Use /plugin on/off <plugin_name> to enable or disable a plugin.
...
builtin-stars-plugin-help-author-version = ✨ Author: {$author}✨ Version: {$version}
builtin-stars-plugin-help-command-list-title = 🔧 Command list:
...
builtin-stars-plugin-help-command-tip = Tip: Add wake prefix to trigger commands, default is /.
...
builtin-stars-provider-list-tts-title = ## Loaded TTS providers
builtin-stars-provider-list-stt-title = ## Loaded STT providers
builtin-stars-provider-list-llm-switch-tip = Use /provider <index> to switch LLM provider.
builtin-stars-provider-list-tts-switch-tip = Use /provider tts <index> to switch TTS provider.
builtin-stars-provider-list-stt-switch-tip = Use /provider stt <index> to switch STT provider.
builtin-stars-provider-list-reachability-skipped = Provider reachability checks were skipped. Enable them in config if needed.
...
builtin-stars-provider-model-current = Current model: [{$current_model}]
builtin-stars-provider-model-switch-tip = Tip: use /model <model_name/index> to switch model in real time. If the target model is not listed, input the model name directly.
...
builtin-stars-provider-key-current = Current key: {$current_key}
builtin-stars-provider-model-current-inline = Current model: {$current_model}
builtin-stars-provider-key-switch-tip = Use /key <idx> to switch key.
...
builtin-stars-sid-base-info = UMO: "{$sid}" This value can be used for whitelist settings.UID: "{$user_id}" This value can be used for admin settings.Message session source info: Bot ID: "{$umo_platform}" Message type: "{$umo_msg_type}" Session ID: "{$umo_session_id}"This source info can be used for routing config.
builtin-stars-sid-unique-session-group-tip = Unique-session mode is currently enabled. Group ID: "{$group_id}". You can also add this ID to whitelist to allow the whole group.
...
builtin-stars-alter-cmd-usage = This command sets permissions for a command or command group.Format: /alter_cmd <cmd_name> <admin/member>Example 1: /alter_cmd c1 admin sets c1 as admin-only commandExample 2: /alter_cmd g1 c1 admin sets sub-command c1 in group g1 as admin-only/alter_cmd reset config opens reset permission config
builtin-stars-alter-cmd-reset-config-menu = Fine-grained permission config for reset commandCurrent config:1. Group + unique session ON: {$group_unique_on}2. Group + unique session OFF: {$group_unique_off}3. Private chat: {$private}Update format:/alter_cmd reset scene <scene_index> <admin/member>Example: /alter_cmd reset scene 2 member
...
builtin-stars-persona-overview = [Persona]- Persona list: `/persona list`- Set persona: `/persona <persona_name>`- Persona details: `/persona view <persona_name>`- Unset persona: `/persona unset`Default persona: {$default_persona_name}Persona of current conversation {$curr_cid_title}: {$curr_persona_name}Configure personas in WebUI -> Config page
builtin-stars-persona-list-title = 📂 Persona list:
builtin-stars-persona-list-total = Total personas: {$total_count}
builtin-stars-persona-list-set-tip = *Use `/persona <persona_name>` to set persona
...
builtin-stars-conversation-history-result = Current conversation history: {$history}Page {$page} | Total {$total_pages}*Input /history 2 to jump to page 2
...
builtin-stars-conversation-list-line = {$index}. {$title}({$cid}) Persona: {$persona_id} Updated at: {$updated_at}
builtin-stars-conversation-list-divider = ---
builtin-stars-conversation-current-with-id = Current conversation: {$title}({$cid})
builtin-stars-conversation-current-none = Current conversation: None
builtin-stars-conversation-scope-personal = Session isolation scope: Personal
builtin-stars-conversation-scope-group = Session isolation scope: Group
builtin-stars-conversation-page-info = Page {$page} | Total {$total_pages}
builtin-stars-conversation-page-jump-tip = *Input /ls 2 to jump to page 2
...
builtin_stars-astrbot-metadata-desc = desc: Built-in plugin for AstrBot, implementing features such as personality injection, thinking content injection, group chat context awareness, etc. Disabling it will prevent the use of these features.
builtin_stars-builtin_commands-commands-utils-rst_scene-group_unique_on = Group + Session Isolation Enabled
builtin_stars-builtin_commands-commands-utils-rst_scene-group_unique_off = Group + Session Isolation Disabled
builtin_stars-builtin_commands-commands-utils-rst_scene-private = Private Chat
builtin_stars-builtin_commands-metadata-desc = desc: Built-in commands for AstrBot, providing common functions such as conversation management, tool usage, plugin management, etc.
builtin_stars-session_controller-main-social_media_wakeup_note = Note: You are currently chatting with a user on social media. The user only woke you up with @, but did not input any content in this message. They may send what they want to say in the next message.
builtin_stars-session_controller-main-ask_user_intent_friendly = You should friendly ask the user what they would like to talk about or what help they need. Reply in a way that matches your character, and avoid being too mechanical.
builtin_stars-session_controller-main-output_only_reply_content = Please note: You only need to output the content to reply to the user, and nothing else.
builtin_stars-session_controller-metadata-desc = desc: Adds session control support for plugins
builtin_stars-web_searcher-main-migrate_old_tavily_key = Detected old version websearch_tavily_key (string format), automatically migrating to list format and saving.
builtin_stars-web_searcher-main-missing_tavily_key_error = Error: Tavily API key is not configured in AstrBot.
builtin_stars-web_searcher-main-command_deprecated_use_webui = This command has been deprecated. Please enable or disable web search functionality in the WebUI.
builtin_stars-web_searcher-main-summarize_with_bullets_and_links = \n\nFor the question, please summarize the above results in bullet points, and include reference links to the corresponding content at the end (if available).
builtin_stars-web_searcher-main-missing_bocha_key_error = Error: BoCha API key is not configured in AstrBot.
builtin_stars-web_searcher-metadata-desc = desc: Enables web search and retrieval capabilities for the LLM
builtin_stars-session_controller-main-friendly_inquiry = Friendly ask the user what they would like to talk about or what help they need. Reply in a way that matches your persona, avoid being too robotic.
builtin_stars-session_controller-main-reply_only_instruction = Please note: You only need to output the content to reply to the user, do not output anything else.
builtin_stars-web_searcher-main-tavily_key_migration = Detected old format websearch_tavily_key (string format), automatically migrating to list format and saving.
builtin_stars-web_searcher-main-tavily_key_missing = Error: Tavily API key is not configured in AstrBot.
builtin_stars-web_searcher-main-command_deprecated = This command has been deprecated. Please enable or disable web search functionality in the WebUI.
builtin_stars-web_searcher-main-summary_instruction = \n\nFor the question, please summarize the above results in bullet points, and include reference links to the corresponding content at the end (if available).
builtin_stars-web_searcher-main-bocha_key_missing = Error: BoCha API key is not configured in AstrBot.
