...
dashboard-utils-tsne-title = t-SNE 可视化：知识库中的查询({$n} 个向量, {$d} 维, 知识库：{$kb_name})
...
dashboard-server-port-in-use = 错误：端口 {$port} 已被占用占用信息： {$info}请确保：1. 没有其他 AstrBot 实例正在运行2. 端口 {$port} 没有被其他程序占用3. 如需使用其他端口，请修改配置文件
dashboard-server-started-banner = ✨✨✨ AstrBot v{$version} WebUI 已启动，可访问
