...
builtin-stars-web-searcher-link-summary-instruction = この質問について、上記の結果を基に要点を箇条書きで要約し、可能であれば末尾に参照リンクを付けてください。
...
builtin-stars-plugin-list-footer = /plugin help <plugin_name> でプラグインヘルプと登録コマンドを確認できます。/plugin on/off <plugin_name> でプラグインを有効化/無効化できます。
...
builtin-stars-plugin-help-author-version = ✨ 作者：{$author}✨ バージョン: {$version}
builtin-stars-plugin-help-command-list-title = 🔧 コマンド一覧：
...
builtin-stars-plugin-help-command-tip = ヒント: コマンド実行にはウェイクプレフィックスが必要です。既定値は / です。
...
builtin-stars-provider-list-tts-title = ## 読み込み済み TTS プロバイダー
builtin-stars-provider-list-stt-title = ## 読み込み済み STT プロバイダー
builtin-stars-provider-list-llm-switch-tip = /provider <index> で LLM プロバイダーを切り替えます。
builtin-stars-provider-list-tts-switch-tip = /provider tts <index> で TTS プロバイダーを切り替えます。
builtin-stars-provider-list-stt-switch-tip = /provider stt <index> で STT プロバイダーを切り替えます。
builtin-stars-provider-list-reachability-skipped = プロバイダー到達性チェックはスキップされました。必要なら設定で有効化してください。
...
builtin-stars-provider-model-current = 現在のモデル: [{$current_model}]
builtin-stars-provider-model-switch-tip = ヒント: /model <model_name/index> でリアルタイムにモデルを切り替えられます。対象モデルが一覧にない場合はモデル名を直接入力してください。
...
builtin-stars-provider-key-current = 現在のキー: {$current_key}
builtin-stars-provider-model-current-inline = 現在のモデル: {$current_model}
builtin-stars-provider-key-switch-tip = /key <idx> でキーを切り替えます。
...
builtin-stars-sid-base-info = UMO: "{$sid}" この値はホワイトリスト設定に使用できます。UID: "{$user_id}" この値は管理者設定に使用できます。メッセージセッションの送信元情報： Bot ID: "{$umo_platform}" メッセージ種別："{$umo_msg_type}" セッション ID: "{$umo_session_id}"この送信元情報はルーティング設定に使用できます。
builtin-stars-sid-unique-session-group-tip = 現在ユニークセッションモードが有効です。グループ ID: "{$group_id}"。この ID をホワイトリストに追加してグループ全体を許可することもできます。
...
builtin-stars-alter-cmd-usage = このコマンドは、コマンドまたはコマンドグループの権限を設定します。形式：/alter_cmd <cmd_name> <admin/member>例1: /alter_cmd c1 admin で c1 を管理者専用コマンドに設定例2: /alter_cmd g1 c1 admin でグループ g1 のサブコマンド c1 を管理者専用に設定/alter_cmd reset config で reset 権限設定を開く
builtin-stars-alter-cmd-reset-config-menu = reset コマンドの詳細権限設定現在の設定：1. グループ + ユニークセッション ON: {$group_unique_on}2. グループ + ユニークセッション OFF: {$group_unique_off}3. プライベートチャット: {$private}更新形式：/alter_cmd reset scene <scene_index> <admin/member>例：/alter_cmd reset scene 2 member
...
builtin-stars-persona-overview = [Persona]- Persona 一覧：`/persona list`- Persona 設定：`/persona <persona_name>`- Persona 詳細：`/persona view <persona_name>`- Persona 解除：`/persona unset`デフォルト Persona: {$default_persona_name}現在の会話 {$curr_cid_title} の Persona: {$curr_persona_name}Persona は WebUI -> Config ページで設定してください
builtin-stars-persona-list-title = 📂 Persona 一覧：
builtin-stars-persona-list-total = Persona 合計：{$total_count}
builtin-stars-persona-list-set-tip = *`/persona <persona_name>` で Persona を設定
...
builtin-stars-conversation-history-result = 現在の会話履歴：{$history}Page {$page} | Total {$total_pages}*/history 2 で 2 ページ目へ移動
...
builtin-stars-conversation-list-line = {$index}. {$title}({$cid}) Persona: {$persona_id} 更新日時：{$updated_at}
builtin-stars-conversation-list-divider = ---
builtin-stars-conversation-current-with-id = 現在の会話：{$title}({$cid})
builtin-stars-conversation-current-none = 現在の会話：なし
builtin-stars-conversation-scope-personal = セッション分離スコープ: 個人
builtin-stars-conversation-scope-group = セッション分離スコープ: グループ
builtin-stars-conversation-page-info = Page {$page} | Total {$total_pages}
builtin-stars-conversation-page-jump-tip = */ls 2 で 2 ページ目へ移動
...
builtin_stars-builtin_commands-commands-utils-rst_scene-group_unique_on = 群聊+会話隔離を有効化
builtin_stars-builtin_commands-commands-utils-rst_scene-group_unique_off = 群聊+会話隔離を無効化
builtin_stars-builtin_commands-commands-utils-rst_scene-private = プライベートチャット
builtin_stars-builtin_commands-metadata-desc = desc: AstrBot 自带指令，提供常用的对话管理、工具使用、插件管理等功能。
builtin_stars-session_controller-main-social_media_wakeup_note = 注意、現在ソーシャルメディア上でユーザーとチャットしています。ユーザーは@であなたを呼び出しただけで、このメッセージには内容が入力されていません。次のメッセージで内容を送ってくる可能性があります。
builtin_stars-session_controller-main-ask_user_intent_friendly = ユーザーが何を話したいか、どのような助けが必要かを優しく尋ねてください。返信はキャラクター設定に沿った自然なものにし、機械的になりすぎないようにしてください。
builtin_stars-session_controller-main-output_only_reply_content = 注意：ユーザーに返信する内容のみを出力してください。他のものは一切出力しないでください
builtin_stars-session_controller-metadata-desc = desc: プラグインのセッション制御をサポート
builtin_stars-web_searcher-main-migrate_old_tavily_key = 旧形式の websearch_tavily_key（文字列形式）を検出しました。自動的にリスト形式に変換して保存します。
builtin_stars-web_searcher-main-missing_tavily_key_error = エラー：Tavily APIキーがAstrBotで設定されていません。
builtin_stars-web_searcher-main-command_deprecated_use_webui = このコマンドは廃止されました。WebUIでウェブ検索機能を有効/無効にしてください。
builtin_stars-web_searcher-main-summarize_with_bullets_and_links = \n\n問題に対して、上記の結果をもとに箇条書きでまとめ、最後に該当する内容の参考リンク（ある場合）を付けてください。
builtin_stars-web_searcher-main-missing_bocha_key_error = エラー：BoCha APIキーがAstrBotで設定されていません。
builtin_stars-web_searcher-metadata-desc = desc: LLMにウェブ検索機能を付与
builtin_stars-session_controller-main-friendly_inquiry = ユーザーが何を話したいか、どのような助けが必要かを優しく尋ねてください。返信はキャラクター設定に沿った自然なものにし、機械的になりすぎないように。
builtin_stars-session_controller-main-reply_only_instruction = 注意：ユーザーに返信する内容のみを出力してください。それ以外のものは一切出力しないでください。
builtin_stars-web_searcher-main-tavily_key_migration = 旧形式の websearch_tavily_key（文字列形式）を検出しました。自動的にリスト形式に移行して保存します。
builtin_stars-web_searcher-main-tavily_key_missing = エラー：Tavily APIキーがAstrBotで設定されていません。
builtin_stars-web_searcher-main-command_deprecated = このコマンドは廃止されました。WebUIでウェブ検索機能を有効/無効にしてください。
builtin_stars-web_searcher-main-summary_instruction = \n\n問題に対して、上記の結果をもとに箇条書きでまとめ、最後に該当する内容の参考リンク（ある場合）を付けてください。
builtin_stars-web_searcher-main-bocha_key_missing = エラー：BoCha APIキーがAstrBotで設定されていません。
